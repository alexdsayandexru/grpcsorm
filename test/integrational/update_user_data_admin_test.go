...
func UpdateUserDataAdminRequest() *sorm.UpdateUserDataAdminRequest {
	return &sorm.UpdateUserDataAdminRequest{
...
func Test_UpdateUserDataAdminRequest(t *testing.T) {
	grpc := GRPCClient{}
	if err := grpc.Open(); err != nil {
		...
		if client, err := grpc.GetClient(); err != nil {
			t.Error(err)
		} else if resp, _ := client.UpdateUserDataAdmin(grpc.GetCTX(), UpdateUserDataAdminRequest()); resp.Code != 0 {
			t.Error(resp)
		}
	}
}
