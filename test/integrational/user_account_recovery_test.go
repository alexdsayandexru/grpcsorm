...
func UserAccountRecoveryRequest() *sorm.UserAccountRecoveryRequest {
	return &sorm.UserAccountRecoveryRequest{
...
func Test_UserAccountRecoveryRequest(t *testing.T) {
	grpc := GRPCClient{}
	if err := grpc.Open(); err != nil {
		...
		if client, err := grpc.GetClient(); err != nil {
			t.Error(err)
		} else if resp, _ := client.UserAccountRecovery(grpc.GetCTX(), UserAccountRecoveryRequest()); resp.Code != 0 {
			t.Error(resp)
		}
	}
}
